...
| 8 | **Stimulation suppresses spikes on P5; on P1 only two of three detectors see it** | ON/OFF rate ratio over analysable time, whole recordings, **after** the artefact fill: **P5 0.57 / 0.62 / 0.55, all significant** (p 4e-13 / 1e-06 / 4e-12). **P1 0.42 / 0.42 / 0.62**, and Delphos's is **not significant** (p 0.84). Note the fill *raised* P1's Delphos ratio from 0.27 to 0.62 — the contamination was inflating its stim-**OFF** rate, not its ON rate, so removing it made the effect smaller rather than larger. P5 is the recording that establishes the effect; P1 alone would not — `stim_effect.png` |
