...
| 9 | **Delphos's apparent immunity to stimulation was stim artefact** | Its P1 ratio moved 0.90 → 0.27 when the median filter was applied to its input. That is the mechanism established in `lost_to_median.py`: stim produces ~1 ms impulses, Delphos fires on them, the median removes them. The detector that looked unaffected by stimulation was substantially counting glitches. Caveat: P1's Delphos Wilcoxon p is 0.75 (median channel 0.49, 155/223 down), so the pooled 0.27 is driven by busy channels, not a broad per-channel effect; P5's is 6e-12 |
