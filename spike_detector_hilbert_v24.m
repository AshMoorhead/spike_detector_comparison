function [out, discharges, d_decim, envelope, background, envelope_pdf]=spike_detector_hilbert_v24(d,fs,settings)
...
% recomended calling:
% [...]=spike_detector_hilbert_v24(d,fs);
% [...]=spike_detector_hilbert_v24('c:/datafile.mat "sig_var"',fs); % "name of signal matrix"
% [...]=spike_detector_hilbert_v24('c:/datafile.mat "sig_var" "fs_var"'); % "name of signal matrix" and "name of fs value" 
% or
% [...]=spike_detector_hilbert_v24(d,fs,settings);
% _________________________________________________________________________
%
%
% inputs: -----------------------------------------------------------------
...
%   -fl low frequency of filtering ('-fl 10' DEFAULT)
%   -fh high frequency of filtering ('-fh 60' DEFAULT)
...
%   -br autoregresive model order of beta-activity detector ('-br 12' DEFAULT)
%   -dt time between spikes in events (5ms DEFAULT)
%   -pt polyspike union time: spike in same channel nearest then time will
%        be united (120 ms DEFAULT)
...
%           discharges.MRAW ... amplitude of raw signal
%
% required toolboxes:
...
obvious_M=sum(markers_high,2)>0;
...
%                 if sum(ovious_M(round(idx(i)-0.01*fs:idx(i)-0.01*fs)))>0
                if sum(obvious_M(round(idx(i)-discharge_tol*fs:idx(i)+discharge_tol*fs)))>0    
...
%             
    ...
    dis(k).MW=mw;
    dis(k).MPDF=mpdf;
    dis(k).MD=repmat(point(k,2)-point(k,1),1,size(d,2))/fs;
    dis(k).MP=mp/fs;
...
discharges.MV=zeros(itt,size(dis(1).MRAW,2)); % spike type 1-obvious, 0.5- ambiguous
discharges.MA=zeros(itt,size(dis(1).MRAW,2)); % max. amplitude of envelope above backround
discharges.MP=zeros(itt,size(dis(1).MRAW,2)); % event start position
discharges.MD=zeros(itt,size(dis(1).MRAW,2)); % event duration 
discharges.MW=zeros(itt,size(dis(1).MRAW,2)); % statistical weight
discharges.MPDF=zeros(itt,size(dis(1).MRAW,2)); % statistical weight
discharges.MRAW=zeros(itt,size(dis(1).MRAW,2)); % max. amplitude of signal

m1s=0;
for k=1:length(dis)
    m2s=size(dis(k).MRAW,1);
    discharges.MRAW(m1s+1:m1s+m2s,:,:)=dis(k).MRAW;
    discharges.MV(m1s+1:m1s+m2s,:,:)=dis(k).MV;
    discharges.MA(m1s+1:m1s+m2s,:,:)=dis(k).MA;
    discharges.MW(m1s+1:m1s+m2s,:,:)=dis(k).MW;
    discharges.MPDF(m1s+1:m1s+m2s,:,:)=dis(k).MPDF;
    discharges.MD(m1s+1:m1s+m2s,:,:)=dis(k).MD;
    discharges.MP(m1s+1:m1s+m2s,:,:)=dis(k).MP; 
    m1s=m1s+m2s;
end
